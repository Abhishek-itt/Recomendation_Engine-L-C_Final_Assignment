...
bool AdminController::adminLogin(std::string username, int employeeId) {
    UserDTO userDTO;
    userDTO.user_name = username;
    userDTO.employee_id = employeeId;

    std::string serializedData = userDTO.serializeDataAsNSV();

    std::string requestStirng = std::to_string(static_cast<int>(RequestType::ADMIN_LOGIN)) + "\n" + serializedData + "\n";

    clientSocketController.sendMessage(requestStirng);
